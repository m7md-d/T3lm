 ...
 *
 * والفصول الأخرى مكانها المدخل. وما يهمّ القارئ وهو داخل فصلٍ أن يرى أين هو
 * منه، وأن يعود إلى أي قسمٍ مرّ عليه بضغطة.
 *
...
export function Rail({ region, at, nextLabel, onPrev, onNext, go }: {
  region: Region;
  at: number;
  nextLabel?: string;
  onPrev: () => void;
  ...
  return (
    <aside className={`rail${open ? ' rail--open' : ''}`} aria-label="محتويات الفصل">
      <div className="rail__top">
        ...
        <button type="button" className="rail__more" aria-expanded={open} onClick={() => setOpen(!open)}>
          <List aria-hidden />
        </button>
      </div>
