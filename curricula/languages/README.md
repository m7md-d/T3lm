# اللغات

لغات البرمجة — نظام الأنواع والتمثيل والتنفيذ

<!-- doctor:list:start -->
| المنهج | الموضوع | الموقع | الأسلوب | الحالة |
|---|---|---|---|---|
| [`c`](c) | مدخل البرمجة من الآلة صعوداً — لمن لم يكتب سطراً قبل اليوم | [زُر ↗](https://c.nodeksa.com) | `default` | ✅ |
| [`c-advanced`](c-advanced) | الجزء الثاني: virtual memory وallocator وABI وlinker وloader وthreads | [زُر ↗](https://c-advanced.nodeksa.com) | `default` | ✅ |
| [`dart`](dart) | تفكيك Dart من نظام أنواعها إلى حلقة أحداثها وisolatها — لمن أنهى C ولم يفتح Dart | [زُر ↗](https://dart.nodeksa.com) | `default` | ✅ |
| [`go`](go) | تفكيك Go من نظام أنواعها إلى تمثيلها في الذاكرة — لمن يعرف C | [زُر ↗](https://go.nodeksa.com) | `default` | ✅ |
| [`python`](python) | ما الذي يحدث تحت كل سطر Python تكتبه: الاسم والكائن، والمفسّر، والمصفوفة، ودالّة C يناديها خطُّ Python | [زُر ↗](https://python.nodeksa.com) | `default` | 🔨 |
| [`rust`](rust) | تفكيك Rust من المِلكية إلى نظام أنواعها — لمن يعرف C ولم يكتب Rust | [زُر ↗](https://rust.nodeksa.com) | `default` | ✅ |

**مقترحات** — موضوعٌ محجوزٌ بريدمي يقول متطلّباته، بلا منهجٍ بعد:

- [`typescript`](typescript/README.md)
<!-- doctor:list:end -->

البنية والاتفاقات: [`.claude/conventions/repo-layout.md`](../../.claude/conventions/repo-layout.md)
