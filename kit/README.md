...
| `@t3lm/kit/highlight/asm` · `/c` · `/dart` · `/go` · `/rust` · `/sh` | `highlightToHtml(code, lang?)` |
...
أخرى تُعرَض. و`sh` يحمل `dockerfile` للسبب نفسه، و`asm` يحمل `gasArm` لأن مقارنة
معماريتين تعرض المقطعين متجاورين.
...
├── highlight/  index.ts (المحرّك) · asm.ts · c.ts · dart.ts · go.ts · rust.ts · sh.ts
